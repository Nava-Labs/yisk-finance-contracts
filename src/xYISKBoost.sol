...
contract xYISKBoost is Ownable {
  xYISKLockSetting[] public xYISKLockSettings;
  mapping(address => LockStatus) public userLockStatus;

  // Define a struct for the lock settings
  struct xYISKLockSetting {
    uint256 duration;
    uint256 miningBoost;
  }
  ...
  constructor() {
    xYISKLockSettings.push(xYISKLockSetting(30 days, 20 * 1e18));
    xYISKLockSettings.push(xYISKLockSetting(90 days, 30 * 1e18));
    xYISKLockSettings.push(xYISKLockSetting(180 days, 50 * 1e18));
    xYISKLockSettings.push(xYISKLockSetting(365 days, 100 * 1e18));
  }

  // Function to add a new lock setting
  function addLockSetting(xYISKLockSetting memory setting) external onlyOwner {
    xYISKLockSettings.push(setting);
  }

  // Function to set the user's lock status
  function setLockStatus(uint256 id) external {
    xYISKLockSetting memory _setting = xYISKLockSettings[id];
